...
ReloadActor(actorid)
{
    if(ActorInfo[actorid][actorExists])
    {
        ...
        DestroyDynamic3DTextLabel(ActorInfo[actorid][actor_Label1]);
        format(string, sizeof(string), "%s", name);
        ActorInfo[actorid][actor_Label] = CreateDynamic3DTextLabel(string, COLOR_WHITE, ActorInfo[actorid][actorX], ActorInfo[actorid][actorY], ActorInfo[actorid][actorZ] + 1, 5.0);
        ActorInfo[actorid][actor_Label1] = CreateDynamic3DTextLabel(""WHITE"Press "SVRCLR"'N' "WHITE"to interact.", COLOR_GREY, ActorInfo[actorid][actorX], ActorInfo[actorid][actorY], ActorInfo[actorid][actorZ], 5.0);
...
CMD:removeactor(playerid, params[])
{
    ...
    SendMessage(playerid, SERVER_COLOR, "** You have removed actor %i.", actorid);
    return 1;
}
...
CMD:editactor(playerid, params[])
{
    ...
    if(!strcmp(option, "message", true))
    {
        ...
        DestroyActor(ActorInfo[actorid][actor_ID]);
        ReloadActor(actorid);
        SendMessage(playerid, SERVER_COLOR, "** You've changed the name of Actor %i to %s.", actorid, name);
    }
    else if(!strcmp(option, "skin", true))
    {
        ...
        SendMessage(playerid, SERVER_COLOR, "** You've changed the skin of Actor %i to %d.", actorid, skin);
